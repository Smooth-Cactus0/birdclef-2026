# %%
# ============================================================================
# BirdCLEF 2026 -- nb14d: Self-Attention Temporal Context + MLP Probes (Augment)
# ============================================================================
# Architecture (Option C -- augment: keep nb13 scalars, add attention context):
#   Same as nb14c but temporal model is self-attention instead of BiGRU.
#   Input to MLP: [PCA(64) | scalars(5) | ctx(8)] = 77-dim
#   Extra diagnostic: 12x12 attention weight heatmap (attn_weights_nb14d.png)
#
# Inputs:
...
#
# Internet : ON  |  Compute : CPU
# ============================================================================

# %%
import glob as _glob, subprocess as _sub, sys as _sys
_whl = [w for w in _glob.glob("/kaggle/input/**/onnxruntime*cp312*x86_64*.whl", recursive=True) if "gpu" not in w]
if _whl: _sub.check_call([_sys.executable, "-m", "pip", "install", _whl[0], "--quiet"])
else:    _sub.check_call([_sys.executable, "-m", "pip", "install", "onnxruntime", "--quiet"])

# %%
import gc, json, math, re, time, warnings
...
# %%
PERCH_SR = 32_000; WINDOW_SEC = 5; WINDOW_SAMPLES = PERCH_SR * WINDOW_SEC
SC_FILE_SEC = 60;  SC_N_WINDOWS = SC_FILE_SEC // WINDOW_SEC

BATCH_FILES_TRAIN = 16; BATCH_FILES_TEST = 8; IO_WORKERS = 4

PCA_DIM    = 64
SCALAR_DIM = 5
ATTN_DIM   = 64; ATTN_HEADS = 4; ATTN_FF = 128
CTX_DIM    = 8
ATTN_DROP  = 0.1
MLP_IN_DIM = PCA_DIM + SCALAR_DIM + CTX_DIM   # 77

N_FOLDS = 5; EPOCHS = 30; LR = 1e-3; WD = 1e-4; BATCH_SZ = 128; ALPHA = 0.7

FNAME_RE = re.compile(r"BC2026_(?:Train|Test)_(\d+)_(S\d+)_(\d{8})_(\d{6})\.ogg")
def parse_fname(name):
    m = FNAME_RE.match(Path(name).name)
    return {"site": m.group(2), "hour_utc": int(m.group(4)[:2])} if m else {"site": "unknown", "hour_utc": -1}
...
sc = (sc_labels.groupby(["filename","start","end"])["primary_label"]
...
SESS = ort.InferenceSession(str(ONNX_PATH), sess_options=_so, providers=["CPUExecutionProvider"])
        ...
        for start in range(0, n_files, batch_files):
            ...
            for bi, path in enumerate(batch_paths):
                ...
                for wi in range(SC_N_WINDOWS):
                    ...
                    meta_rows.append({"filename": path.name, "window_idx": wi, "end_sec": end_s,
                                      "row_id": f"{path.stem}_{end_s}", "site": meta["site"], "hour_utc": meta["hour_utc"]})
            rows_this = len(batch_audio) * SC_N_WINDOWS
            scores_all[wr:wr+rows_this] = scores; embs_all[wr:wr+rows_this] = embs; wr += rows_this
...
train_files = sorted([f for f in (BASE_DIR/"train_soundscapes").glob("*.ogg") if f.name in labeled_fnames])
...
# %%
Y_TR = np.stack([Y_SC_LOOKUP.get(rid, np.zeros(N_CLASSES, dtype=np.float32)) for rid in meta_tr["row_id"]])
print(f"Labeled windows: {int((Y_TR.sum(1)>0).sum())} / {len(Y_TR)}")
assert int((Y_TR.sum(1)>0).sum()) >= 600
...
def build_temporal_features(scores, meta_df, n_windows=SC_N_WINDOWS):
    N = len(scores); out = np.zeros((N, N_CLASSES, 5), dtype=np.float32)
...
class SelfAttentionContext(nn.Module):
    """1-layer Transformer encoder -> (12, CTX_DIM). Saves attention weights on request."""
    ...
    def forward(self, seq, return_attn=False):
        x = self.pe(seq)
        attn_out, attn_w = self.attn(x, x, x, need_weights=return_attn, average_attn_weights=True)
        if return_attn: self.last_attn_weights = attn_w.detach().cpu()
...
class VectorizedMLP(nn.Module):
    """234 probes on 77-dim [PCA(64)|scalars(5)|ctx(8)] via bmm."""
...
def get_context(attn_model, fids, wids, seq_tensor):
    ...
    ctx_out = attn_model(seq_tensor[unique_fids])
    return ctx_out[inverse, wids]

def build_mlp_input(pca_b, scalars_b, ctx_b):
    B = pca_b.shape[0]
    p = pca_b.unsqueeze(1).expand(B, N_CLASSES, PCA_DIM)
    c = ctx_b.unsqueeze(1).expand(B, N_CLASSES, CTX_DIM)
    return torch.cat([p, scalars_b, c], dim=-1)

def train_one_fold(pca_tr_f, scalars_tr_f, Y_tr_f, fids_tr_f, wids_tr_f,
                   pca_va_f, scalars_va_f, Y_va_f, fids_va_f, wids_va_f,
    ...
    mlp   = VectorizedMLP()
    opt   = torch.optim.Adam(list(attn.parameters()) + list(mlp.parameters()), lr=LR, weight_decay=WD)
    ...
    for ep in range(n_epochs):
        ...
        for i in range(0, n_tr, batch_size):
            ...
            xp_b = Xp_tr[idx]; xs_b = Xs_tr[idx]; yb = Yt_tr[idx]
            fid_b = fids_tr_t[idx]; wid_b = wids_tr_t[idx]
            opt.zero_grad()
            ctx    = get_context(attn, fid_b, wid_b, seq_tensor)
            logits = mlp(build_mlp_input(xp_b, xs_b, ctx))
        ...
        if verbose and ((ep+1) % 10 == 0 or ep == 0):
            ...
            with torch.no_grad():
                ...
                vl = (bce(mlp(build_mlp_input(Xp_va, Xs_va, ctx_va)), Yt_va) * cls_w).mean().item()
            print(f"    ep {ep+1:3d}  train={ep_loss/n_batches:.4f}  val_loss={vl:.4f}")

    attn.eval(); mlp.eval()
    with torch.no_grad():
        ctx_va      = get_context(attn, fids_va_t, wids_va_t, seq_tensor)
        val_pred_on = torch.sigmoid(mlp(build_mlp_input(Xp_va, Xs_va, ctx_va))).numpy()
        ctx_zero    = torch.zeros(len(Xp_va), CTX_DIM)
        val_pred_off = torch.sigmoid(mlp(build_mlp_input(Xp_va, Xs_va, ctx_zero))).numpy()
    return attn, mlp, val_pred_on, val_pred_off

# %%
groups = meta_tr["filename"].to_numpy()
gkf = GroupKFold(n_splits=N_FOLDS)
oof_on = np.zeros_like(Y_TR); oof_off = np.zeros_like(Y_TR)
fold_models = []

print(f"\nTraining {N_FOLDS}-fold Attention-augment+MLP on {len(pca_tr)} windows ...")
for fold, (tr_idx, va_idx) in enumerate(gkf.split(pca_tr, Y_TR, groups)):
    t0 = time.time()
    attn, mlp, vp_on, vp_off = train_one_fold(
        pca_tr[tr_idx],  temp_tr[tr_idx],  Y_TR[tr_idx],
        file_idx_tr[tr_idx],  window_idx_tr[tr_idx],
        pca_tr[va_idx],  temp_tr[va_idx],  Y_TR[va_idx],
    ...
    try: auc = roc_auc_score(Y_TR[va_idx][:,val_active], vp_on[:,val_active], average="macro")
...
print(f"\nBottom 10:"); print(auc_df.tail(10).to_string(index=False))
auc_df.to_csv(OUT_DIR / "per_class_auc_nb14d.csv", index=False)
...
print(f"\n{'='*60}")
print(f"OOF AUC  Perch-only             : {auc_perch_g:.4f}")
print(f"OOF AUC  MLP+scalars, ctx off   : {auc_off_g:.4f}  ({auc_off_g-auc_perch_g:+.4f} vs Perch)")
print(f"OOF AUC  MLP+scalars+Attn       : {auc_on_g:.4f}  ({auc_on_g-auc_off_g:+.4f} vs off)")
print(f"{'='*60}")

diagnostics = {
    "notebook": "nb14d", "model": "Attention-augment",
    "oof_auc_perch"        : round(float(auc_perch_g), 6),
    "oof_auc_temporal_off" : round(float(auc_off_g), 6),
    "oof_auc_temporal_on"  : round(float(auc_on_g), 6),
    "delta_ctx"            : round(float(auc_on_g - auc_off_g), 6),
    "delta_mlp_vs_perch"   : round(float(auc_off_g - auc_perch_g), 6),
    "n_active_classes": int(active_mask.sum()), "n_train_windows": int(len(Y_TR)),
    "n_train_files": int(n_train_files), "attn_dim": ATTN_DIM,
    "attn_heads": ATTN_HEADS, "ctx_dim": CTX_DIM, "mlp_in_dim": MLP_IN_DIM, "epochs": EPOCHS,
}
with open(OUT_DIR / "diagnostics_nb14d.json", "w") as f:
    json.dump(diagnostics, f, indent=2)
print("Saved diagnostics_nb14d.json")
...
fig.suptitle("nb14d Attention (augment): Prediction Distributions — top-10 & bottom-10 by OOF AUC", fontsize=11)
...
plt.tight_layout()
plt.savefig(OUT_DIR / "pred_dist_nb14d.png", dpi=100, bbox_inches="tight")
plt.close(); print("Saved pred_dist_nb14d.png")

# %%
# -- Attention weight heatmap (fold 0, avg across all training files) ---------
attn_fold0 = fold_models[0][0]
attn_fold0.eval()
with torch.no_grad():
    _ = attn_fold0(file_seq_tensor, return_attn=True)
    attn_w = attn_fold0.last_attn_weights   # (n_files, 12, 12)
avg_attn = attn_w.mean(0).numpy()

fig, ax = plt.subplots(figsize=(7, 6))
im = ax.imshow(avg_attn, cmap="Blues", aspect="auto")
ax.set_xlabel("Key position (window)", fontsize=10)
ax.set_ylabel("Query position (window)", fontsize=10)
ax.set_title("nb14d Attn-augment: avg 12×12 weights across training files (fold 0)", fontsize=10)
...
plt.tight_layout()
plt.savefig(OUT_DIR / "attn_weights_nb14d.png", dpi=100, bbox_inches="tight")
plt.close()
print("Saved attn_weights_nb14d.png")
print(f"Diagonal mean (local): {np.diag(avg_attn).mean():.4f}")
print(f"Off-diagonal mean    : {(avg_attn.sum()-np.trace(avg_attn))/(12*11):.4f}")

# %%
del pca_tr, scores_tr, perch_sig_tr, temp_tr, file_seq_np
gc.collect()
...
if not test_files:
    print("test_soundscapes empty -- staging fallback")
...
print(f"OOF AUC  Attn off    : {auc_off_g:.4f}")
print(f"OOF AUC  Attn on     : {auc_on_g:.4f}")
print(f"Attn delta           : {auc_on_g - auc_off_g:+.4f}")
print(sub.head(3))
