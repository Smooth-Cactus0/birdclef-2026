# %%
# ============================================================================
# BirdCLEF 2026 -- nb15f: Per-Class Logit + Global Logit Ctx + Alpha-Blend OFF
# ============================================================================
...
#
# Architecture:
#   [PCA(64) | scalars(5) | BiGRU ctx(8) | per-class logit(1) | global logit ctx(32)] = 110-dim
#   -> 234 VectorizedMLP probes via bmm
#   -> NO alpha-blend: MLP output is final (BLEND OFF)
#
# Ablation OFF: zero both logit features -> approximates nb14c baseline.
...
MLP_IN_DIM    = PCA_DIM + SCALAR_DIM + CTX_DIM + LOGIT_DIM + LOGIT_PCA_DIM  # 110

BLEND      = False   # alpha-blend OFF
N_FOLDS    = 5
EPOCHS     = 30
LR         = 1e-3
WD         = 1e-4
...
# %%
# -- Per-class logit feature + Global logit context PCA(32) ------------------
logit_feat_tr = scores_tr.astype(np.float32)   # (N, 234)
...
print(f"Per-class logit: {logit_feat_tr.shape}  |  Global logit ctx: {logit_ctx_tr.shape}")
...
class VectorizedMLP(nn.Module):
    """234 probes on 110-dim input [PCA(64)|scalars(5)|ctx(8)|logit(1)|glogit(32)] via bmm."""
...
def build_mlp_input(pca_b, scalars_b, ctx_b, logit_b, logit_ctx_b):
    """[PCA(64) | scalars(5) | BiGRU ctx(8) | logit(1) | global logit ctx(32)] -> (B, 234, 110)."""
    B = pca_b.shape[0]
    ...
    return torch.cat([p, scalars_b, c, lb, lc], dim=-1)               # (B, 234, 110)


def train_one_fold(pca_tr_f, scalars_tr_f, logit_tr_f, logit_ctx_tr_f, Y_tr_f, fids_tr_f, wids_tr_f,
                   pca_va_f, scalars_va_f, logit_va_f, logit_ctx_va_f, Y_va_f, fids_va_f, wids_va_f,
    ...
    Xlc_tr = torch.from_numpy(logit_ctx_tr_f).float()  # (N_tr, 32)
    ...
    Xlc_va = torch.from_numpy(logit_ctx_va_f).float()
    ...
    for ep in range(n_epochs):
        ...
        for i in range(0, n_tr, batch_size):
            ...
            logits = mlp(build_mlp_input(xp_b, xs_b, ctx, xl_b, Xlc_tr[idx]))
        ...
        if verbose and ((ep + 1) % 10 == 0 or ep == 0):
            ...
            with torch.no_grad():
                ...
                vl = (bce(mlp(build_mlp_input(Xp_va, Xs_va, ctx_va, Xl_va, Xlc_va)), Yt_va) * cls_w).mean().item()
            print(f"    ep {ep+1:3d}  train={ep_loss/n_batches:.4f}  val_loss={vl:.4f}")

    bigru.eval(); mlp.eval()
    with torch.no_grad():
        ctx_va      = get_context(bigru, fids_va_t, wids_va_t, seq_tensor)
        val_pred_on = torch.sigmoid(mlp(build_mlp_input(Xp_va, Xs_va, ctx_va, Xl_va, Xlc_va))).numpy()
        # Ablation: zero both logit features -> approximates nb14c (scalars + BiGRU only)
        logit_zero     = torch.zeros_like(Xl_va)
        logit_ctx_zero = torch.zeros_like(Xlc_va)
        val_pred_off   = torch.sigmoid(mlp(build_mlp_input(Xp_va, Xs_va, ctx_va, logit_zero, logit_ctx_zero))).numpy()
...
print(f"\nTraining {N_FOLDS}-fold BiGRU+both-logit MLP on {len(pca_tr)} windows ...")
for fold, (tr_idx, va_idx) in enumerate(gkf.split(pca_tr, Y_TR, groups)):
    t0 = time.time()
    bigru, mlp, vp_on, vp_off = train_one_fold(
        pca_tr[tr_idx],       temp_tr[tr_idx],       logit_feat_tr[tr_idx],  logit_ctx_tr[tr_idx],
        Y_TR[tr_idx],         file_idx_tr[tr_idx],   window_idx_tr[tr_idx],
        pca_tr[va_idx],       temp_tr[va_idx],        logit_feat_tr[va_idx], logit_ctx_tr[va_idx],
...
auc_df.to_csv(OUT_DIR / "per_class_auc_nb15f.csv", index=False)
...
diagnostics = {
    "notebook": "nb15f", "model": "both-logit-blend-off",
    "blend": False, "logit_mode": "both",
    ...
    "logit_pca_dim":    LOGIT_PCA_DIM,
    "epochs":            EPOCHS,
}
with open(OUT_DIR / "diagnostics_nb15f.json", "w") as f:
    json.dump(diagnostics, f, indent=2)
print("Saved diagnostics_nb15f.json")
...
fig.suptitle("nb15f both-logit (blend OFF): Prediction Distributions", fontsize=11)
...
plt.tight_layout()
plt.savefig(OUT_DIR / "pred_dist_nb15f.png", dpi=100, bbox_inches="tight")
plt.close(); print("Saved pred_dist_nb15f.png")

# %%
del pca_tr, scores_tr, perch_sig_tr, temp_tr, logit_feat_tr, logit_ctx_tr, file_seq_np
gc.collect()
...
Xlc_te  = torch.from_numpy(logit_ctx_te).float()
        ...
        for i in range(0, len(Xp_te), chunk):
            out_k[i:i+chunk] = torch.sigmoid(
                mlp_k(build_mlp_input(Xp_te[i:i+chunk], Xs_te[i:i+chunk],
                                      ctx_te[i:i+chunk], Xl_te[i:i+chunk], Xlc_te[i:i+chunk]))
            ).numpy()
        mlp_pred += out_k; print(f"  fold {k} done")
mlp_pred /= len(fold_models)

# %%
# BLEND OFF: MLP prediction is the final output
final = mlp_pred
