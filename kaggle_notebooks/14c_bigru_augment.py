# %%
# ============================================================================
# BirdCLEF 2026 -- nb14c: BiGRU Temporal Context + MLP Probes (Augment)
# ============================================================================
# Architecture (Option C -- augment: keep nb13 scalars, add BiGRU context):
#   1. Perch v2 ONNX: extract embeddings + logits on 66 labeled soundscapes
#   2. PCA(64): compress 1536-dim embeddings
#   3. Hand-crafted scalars: (N, 234, 5) prev/next/mean/max/std of Perch logit
#   4. File sequence matrix: (n_files, 12, 64) PCA embeddings per file
#   5. BiGRU(hidden=32): context -> (n_files, 12, 8)
#   6. VectorizedMLP: 234 probes on [PCA(64) | scalars(5) | ctx(8)] = 77-dim
#   7. Joint training end-to-end
#   8. Alpha-blend with Perch sigmoid
#
# Ablation OFF: zero out ctx(8), keep scalars(5) -> approximates nb13 baseline
# Delta therefore measures learned temporal context on top of nb13 features.
#
# Inputs:
...
# Compute  : CPU (~3 min Perch + ~5 min training + test Perch)
# ============================================================================
...
PCA_DIM    = 64
SCALAR_DIM = 5           # nb13 hand-crafted temporal scalars (kept)
GRU_HIDDEN = 32
CTX_DIM    = 8           # learned BiGRU context (appended)
GRU_DROP   = 0.1
...
print(f"Classes: {N_CLASSES}  |  Soundscape rows: {len(sc_labels)}")
...
# %%
# -- Hand-crafted temporal scalars (5 per class, same as nb13) ----------------
def build_temporal_features(scores, meta_df, n_windows=SC_N_WINDOWS):
    """prev/next/mean/max/std of Perch logit across 12 windows per file.
    Returns (N, 234, 5) class-specific scalars."""
    N = len(scores)
    ...
    for fname, group in meta_df.groupby("filename", sort=False):
        ...
        out[idxs, :, 0] = np.roll(block,  1, 0)   # prev
        out[idxs, :, 1] = np.roll(block, -1, 0)   # next
...
class BiGRUContext(nn.Module):
    """BiGRU over 12-window sequence -> (12, CTX_DIM) context."""
...
class VectorizedMLP(nn.Module):
    """234 probes on 77-dim input [PCA(64)|scalars(5)|ctx(8)] via bmm."""
...
def get_context(bigru, fids, wids, seq_tensor):
    ...
    gru_out = bigru(seq_tensor[unique_fids])   # (n_unique, 12, CTX_DIM)
    return gru_out[inverse, wids]              # (B, CTX_DIM)


def build_mlp_input(pca_b, scalars_b, ctx_b):
    """[PCA(64) | class-specific scalars(5) | shared ctx(8)] -> (B, 234, 77)."""
    B = pca_b.shape[0]
    p = pca_b.unsqueeze(1).expand(B, N_CLASSES, PCA_DIM)    # (B, 234, 64)
    c = ctx_b.unsqueeze(1).expand(B, N_CLASSES, CTX_DIM)    # (B, 234, 8)
    return torch.cat([p, scalars_b, c], dim=-1)              # (B, 234, 77)
...
def train_one_fold(pca_tr_f, scalars_tr_f, Y_tr_f, fids_tr_f, wids_tr_f,
    ...
    Xs_tr = torch.from_numpy(scalars_tr_f).float()   # (N_tr, 234, 5)
    ...
    with torch.no_grad():
        ...
        # Ablation OFF: zero ctx but keep scalars (approximates nb13 within this model)
...
print(f"\nTraining {N_FOLDS}-fold BiGRU-augment+MLP on {len(pca_tr)} windows ...")
...
for i in active_idxs:
    ...
    per_class_rows.append({"species": lbl, "n_pos": int(y_i.sum()),
                            ...
                            "delta_ctx": auc_on - auc_off,      # BiGRU added value
                            "delta_mlp": auc_off - auc_perch})  # nb13 scalars value
...
auc_df.to_csv(OUT_DIR / "per_class_auc_nb14c.csv", index=False)
...
print(f"OOF macro-AUC  MLP+scalars, ctx off: {auc_off_g:.4f}  ({auc_off_g-auc_perch_g:+.4f} vs Perch)")
print(f"OOF macro-AUC  MLP+scalars+BiGRU   : {auc_on_g:.4f}  ({auc_on_g-auc_off_g:+.4f} vs off)")
print(f"{'='*60}")

diagnostics = {
    "notebook": "nb14c", "model": "BiGRU-augment",
...
with open(OUT_DIR / "diagnostics_nb14c.json", "w") as f:
    json.dump(diagnostics, f, indent=2)
print("Saved diagnostics_nb14c.json")
...
fig.suptitle("nb14c BiGRU (augment): Prediction Distributions — top-10 & bottom-10 by OOF AUC", fontsize=11)
...
plt.tight_layout()
plt.savefig(OUT_DIR / "pred_dist_nb14c.png", dpi=100, bbox_inches="tight")
plt.close(); print("Saved pred_dist_nb14c.png")
...
# %%
print("\nRunning 5-fold BiGRU-augment+MLP ensemble on test ...")
...
with torch.no_grad():
    for k, (bigru_k, mlp_k) in enumerate(fold_models):
        ...
        gru_out_te = bigru_k(file_seq_te_tensor)       # (n_test_files, 12, CTX_DIM)
        ctx_te     = gru_out_te[fids_te, wids_te]      # (N_te, CTX_DIM)
...
print(f"OOF AUC  MLP off     : {auc_off_g:.4f}")
print(f"OOF AUC  BiGRU on    : {auc_on_g:.4f}")
print(f"BiGRU delta          : {auc_on_g - auc_off_g:+.4f}")
print(sub.head(3))
