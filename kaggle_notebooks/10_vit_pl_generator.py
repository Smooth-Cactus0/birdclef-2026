...
#   Mel spectrograms are resized to (IMG_SIZE, IMG_SIZE) and repeated 3x before norm
#
# RUNTIME:
...
# 2. Single-channel mel is **repeated 3x** to produce an RGB-like tensor.
# 3. **ImageNet normalization** (?/?) is applied -- ViT LayerNorm was calibrated on these.

# %%
class ViTBirdDataset(Dataset):
    """
    Loads OGG audio -> log-mel spectrogram -> resize to IMG_SIZE x IMG_SIZE ->
...
# - 5-fold training would take ~35h total (7h x 5) -- not worth it
...
for epoch in range(1, CFG['EPOCHS'] + 1):
    ...
    if vl_auc > best_auc:
        ...
        print(f"    OK saved  (best auc={best_auc:.4f})")
