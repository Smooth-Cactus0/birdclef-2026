# %%
# ============================================================================
# BirdCLEF 2026 -- nb14a: BiGRU Temporal Context + MLP Probes (Replace)
# ============================================================================
# Architecture (Option A -- replace temporal scalars with learned BiGRU context):
...
#   4. BiGRU(hidden=32): learns temporal context -> (n_files, 12, 5) per window
#   5. VectorizedMLP: 234 probes on [PCA(64) | GRU_ctx(5)] = 69-dim (same as nb13)
#   6. Joint training: BiGRU + MLP end-to-end
#   7. Alpha-blend with Perch sigmoid (same class-conditional weights as nb13)
#
# Diagnostics:
#   - Per-class AUC table: perch_baseline | temporal_off | temporal_on | delta
#   - Global macro-AUC ablation delta (JSON)
#   - Prediction distribution histograms (top-10 / bottom-10 species by AUC)
#
# Inputs:
...
PCA_DIM    = 64
GRU_HIDDEN = 32         # per-direction; bidirectional -> 64-dim output
CTX_DIM    = 5          # projected context dim; equals nb13's 5 temporal scalars
GRU_DROP   = 0.1
...
WD        = 1e-4
BATCH_SZ  = 128
ALPHA     = 0.7         # MLP weight in alpha-blend
...
# Save Perch sigmoid baseline for diagnostics (before cleanup)
...
del embs_tr   # embeddings no longer needed
gc.collect()

# %%
# -- File sequence matrix: (n_files, 12, 64) ----------------------------------
# Maps each window row to its (file_idx, window_idx) position in the sequence.
...
file_seq_tensor = torch.from_numpy(file_seq_np).float()   # (n_files, 12, 64)
...
class BiGRUContext(nn.Module):
    """Bidirectional GRU over 12-window file sequence -> (12, CTX_DIM) context.

    Processes entire file at once; gradient flows through all 12 positions.
    """
    ...
    def forward(self, seq):
        # seq: (B_files, 12, in_dim) -> (B_files, 12, CTX_DIM)
        h, _ = self.gru(seq)
...
class VectorizedMLP(nn.Module):
    ...
    def forward(self, x):
        # x: (B, n_cls, in_dim)
        x = x.permute(1, 0, 2)                         # (n_cls, B, in_dim)
        h = F.relu(torch.bmm(x,  self.W1) + self.b1)
        h = F.relu(torch.bmm(h,  self.W2) + self.b2)
        return (torch.bmm(h, self.W3) + self.b3).squeeze(-1).t()   # (B, n_cls)

# %%
# -- Training utilities -------------------------------------------------------

def get_context(bigru, fids, wids, seq_tensor):
    """Run BiGRU on unique files in batch; return per-window context (B, CTX_DIM).

    Uses torch.unique return_inverse to avoid re-running GRU on the same file
    within a batch while keeping gradient flow intact.
    """
...
def build_mlp_input(pca_b, ctx):
    """Broadcast [pca | ctx] to (B, N_CLASSES, PCA_DIM+CTX_DIM)."""
    B = pca_b.shape[0]
...
def train_one_fold(pca_tr_f, Y_tr_f, fids_tr_f, wids_tr_f,
    ...
    opt   = torch.optim.Adam(
        list(bigru.parameters()) + list(mlp.parameters()), lr=LR, weight_decay=WD)
    ...
    # OOF predictions: temporal ON and OFF (ablation)
    bigru.eval(); mlp.eval()
    with torch.no_grad():
        ctx_va      = get_context(bigru, fids_va_t, wids_va_t, seq_tensor)
...
print(f"\nTraining {N_FOLDS}-fold BiGRU+MLP on {len(pca_tr)} windows "
      f"({n_train_files} files) ...")
...
# Three reference columns: Perch sigmoid-only | MLP temporal OFF | MLP temporal ON
...
for i in active_idxs:
    ...
    per_class_rows.append({
        ...
        "auc_off"   : auc_off,
        "auc_on"    : auc_on,
        "delta_gru" : auc_on - auc_off,          # BiGRU contribution
        "delta_mlp" : auc_off - auc_perch,       # MLP (no temporal) contribution
    })
...
auc_df.to_csv(OUT_DIR / "per_class_auc_nb14a.csv", index=False)
...
print(f"OOF macro-AUC  MLP off      : {auc_off_global:.4f}  (+{auc_off_global-auc_perch_global:+.4f} vs Perch)")
print(f"OOF macro-AUC  BiGRU on     : {auc_on_global:.4f}  ({auc_on_global-auc_off_global:+.4f} vs MLP-off)")
print(f"{'='*55}")
...
diagnostics = {
    ...
    "delta_gru"           : round(float(auc_on_global - auc_off_global), 6),
...
with open(OUT_DIR / "diagnostics_nb14a.json", "w") as f:
    json.dump(diagnostics, f, indent=2)
print("Saved diagnostics_nb14a.json")
...
fig.suptitle(
    "nb14a BiGRU (replace): Prediction Distributions — top-10 & bottom-10 by OOF AUC",
    fontsize=11)
...
plt.tight_layout()
plt.savefig(OUT_DIR / "pred_dist_nb14a.png", dpi=100, bbox_inches="tight")
plt.close()
print("Saved pred_dist_nb14a.png")
...
# %%
# -- 5-fold BiGRU+MLP ensemble on test ----------------------------------------
print("\nRunning 5-fold BiGRU+MLP ensemble on test ...")
...
with torch.no_grad():
    for k, (bigru_k, mlp_k) in enumerate(fold_models):
        ...
        # Run GRU on all test files in one batched pass
...
if in_real_submission:
    ...
    print("STAGING: writing pred_df directly (row_ids won't match dummy sample_sub)")
...
print(f"OOF AUC  MLP off      : {auc_off_global:.4f}")
print(f"OOF AUC  BiGRU on     : {auc_on_global:.4f}")
print(f"BiGRU delta (on-off)  : {auc_on_global - auc_off_global:+.4f}")
print(sub.head(3))
