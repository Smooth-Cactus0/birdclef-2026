# %%
# ============================================================================
# BirdCLEF 2026 -- nb23 PL SAVER R2: 5-fold NS R1 ensemble predictions on
#                                     unlabeled train_soundscapes for Noisy
#                                     Student round 2 training.
# ============================================================================
# Inputs:
#   - alexycactus/birdclef-2026-cnn-ns1-checkpoints    (5 NS R1 fold .pth files)
...
#
# Delta vs R1 saver: (a) checkpoints are NS R1 outputs (better teachers),
# (b) trim threshold relaxed 0.1 -> 0.05 to recover soft-positive signal that
# R1 discarded -- 2025 1st place used progressively softer trims across rounds.
#
# Output: written to /kaggle/working/
#   pl_preds.npy        (n_files, 12, N_CLASSES) float32  -- 12 framewise
#                       slot predictions per file. Per-class probabilities
#                       < 0.05 are trimmed to 0 BEFORE saving (R2 relaxed
#                       from R1's 0.1 trim).
...
pl_preds[pl_preds < 0.05] = 0.0
n_with_signal = int((pl_preds.max(axis=(1, 2)) > 0.0).sum())
print(f"After trim < 0.05: {n_with_signal}/{len(test_files)} files "
